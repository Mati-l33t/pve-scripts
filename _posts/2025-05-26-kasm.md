---
layout: post
title: "Kasm"
date: 2025-05-26 00:00:00 +0000
categories: ["Monitoring & Analytics"]
tags: [kasm, lxc, monitoring-analytics, auto-update, privileged]
description: "Kasm Workspaces is a container streaming platform that delivers browser-based access to desktops, applications, and web services with enhanced security and scalability."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/kasm-workspaces.webp"
#image:
#  path: /assets/img/kasm.png
#  alt: Kasm
---

## Installation

**Default install:**
```bash
bash -c "$(wget -qLO - https://github.com/community-scripts/ProxmoxVE/raw/main/ct/kasm.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 2 cores</span>
  <span class="res-pill res-ram">RAM: 8192 MB</span>
  <span class="res-pill res-disk">Disk: 50 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

## Notes

<div class="warn-callout">
  <i class="fas fa-exclamation-triangle"></i>
  <div>WARNING: Installation sources scripts outside of Community Scripts repo. Please check the source before installing.</div>
</div>

<div class="warn-callout">
  <i class="fas fa-exclamation-triangle"></i>
  <div>Kasm needs swap (on Proxmox host) and activated FUSE to be installed successfully!</div>
</div>

<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>Show credentials: <code>cat ~/kasm.creds</code></div>
</div>

## Web Interface

<div class="resource-bar"><span class="res-pill res-port">Port: 443</span></div>

## Links

- [Official Website](https://www.kasmweb.com/)
- [Documentation](https://www.kasmweb.com/docs/)

---