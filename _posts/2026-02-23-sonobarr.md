---
layout: post
title: "Sonobarr"
date: 2026-02-23 00:00:00 +0000
categories: ["*Arr Suite"]
tags: [sonobarr, lxc, arr-suite, auto-update]
description: "Sonobarr marries your existing Lidarr library with Last.fm’s discovery graph to surface artists you'll actually like. It runs as a Flask + Socket.IO application, ships with a polished Bootstrap UI, and includes admin tooling so folks can share a single instance safely."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/sonobarr.webp"
image:
  path: /assets/img/sonobarr.png
  alt: Sonobarr
---

## Installation

**Default install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/sonobarr.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 1024 MB</span>
  <span class="res-pill res-disk">Disk: 20 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

## Configuration

Config file:
```
/etc/sonobarr/.env
```

## Notes

<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>Default generated admin password is in the env file (sonobarr_superadmin_password)</div>
</div>

## Web Interface

<div class="resource-bar"><span class="res-pill res-port">Port: 5000</span></div>

## Links

- [Official Website](https://github.com/Dodelidoo-Labs/sonobarr)
- [Documentation](https://github.com/Dodelidoo-Labs/sonobarr)

---