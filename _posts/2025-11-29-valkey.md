---
layout: post
title: "Valkey"
date: 2025-11-29 00:00:00 +0000
categories: [Databases]
tags: [valkey, lxc, databases, auto-update]
description: "Valkey is an open source (BSD) high-performance key/value datastore that supports a variety of workloads such as caching, message queues, and can act as a primary database. The project is backed by the Linux Foundation, ensuring it will remain open source forever."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/valkey.webp"
#image:
#  path: /assets/img/valkey.png
#  alt: Valkey
---

## Installation

**Default install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/valkey.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 1024 MB</span>
  <span class="res-pill res-disk">Disk: 4 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

**Alpine install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/alpine-valkey.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 256 MB</span>
  <span class="res-pill res-disk">Disk: 1 GB</span>
  <span class="res-pill res-os">OS: Alpine 3.23</span>
</div>

## Configuration

Config file:
```
/etc/valkey/valkey.conf
```

## Notes

<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>Show Login Credentials, type <code>cat ~/valkey.creds</code> in the LXC console</div>
</div>

<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>Alpines Valkey package is compiled without TLS support. For TLS, use the Debian-based valkey script instead.</div>
</div>

## Web Interface

<div class="resource-bar"><span class="res-pill res-port">Port: 6379</span></div>

## Links

- [Official Website](https://valkey.io/)
- [Documentation](https://valkey.io/docs/)

---