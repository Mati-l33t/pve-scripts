---
layout: post
title: "TasmoAdmin"
date: 2024-05-02 00:00:00 +0000
categories: ["IoT & Smart Home"]
tags: [tasmoadmin, lxc, iot-smart-home, auto-update]
description: "TasmoAdmin is an administrative platform for devices flashed with Tasmota."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/tasmoadmin.webp"
#image:
#  path: /assets/img/tasmoadmin.png
#  alt: TasmoAdmin
---

## Installation

**Default install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/tasmoadmin.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 512 MB</span>
  <span class="res-pill res-disk">Disk: 2 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

## Web Interface

<div class="resource-bar"><span class="res-pill res-port">Port: 9999</span></div>

## Links

- [Official Website](https://github.com/TasmoAdmin/TasmoAdmin#readme)

---