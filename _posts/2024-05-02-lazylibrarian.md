---
layout: post
title: "LazyLibrarian"
date: 2024-05-02 00:00:00 +0000
categories: ["Media & Streaming"]
tags: [lazylibrarian, lxc, media-streaming, updateable]
description: "LazyLibrarian is a SickBeard, CouchPotato, Headphones-like application for ebooks, audiobooks and magazines."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/lazylibrarian.webp"
#image:
#  path: /assets/img/lazylibrarian.png
#  alt: LazyLibrarian
---

## Installation

**Default install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/lazylibrarian.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 2 cores</span>
  <span class="res-pill res-ram">RAM: 1024 MB</span>
  <span class="res-pill res-disk">Disk: 4 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

## Configuration

Config file:
```
/opt/LazyLibrarian/config.ini
```

## Web Interface

<div class="resource-bar"><span class="res-pill res-port">Port: 5299</span></div>

## Links

- [Official Website](https://gitlab.com/LazyLibrarian/LazyLibrarian)
- [Documentation](https://lazylibrarian.gitlab.io/lazylibrarian.gitlab.io/)

---