...
<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>To exclude LXCs from updating, edit the crontab using <code>crontab -e</code> and add CTID as shown in the example below:



<code>0 0 * * 0 PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/pve/update-lxcs-cron.sh)" -s 103 111 >>/var/log/update-lxcs-cron.log 2>/dev/null</code></div>
</div>
