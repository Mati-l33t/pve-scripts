---
layout: post
title: "Redis "
date: 2024-05-02 00:00:00 +0000
categories: [Databases]
tags: [redis, lxc, databases]
description: "Redis is an open-source, in-memory data store used by millions of developers as a cache, vector database, document database, streaming engine, and message broker."
icon: "https://cdn.jsdelivr.net/gh/selfhst/icons@main/webp/redis.webp"
#image:
#  path: /assets/img/redis.png
#  alt: Redis 
---

## Installation

**Default install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/redis.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 1024 MB</span>
  <span class="res-pill res-disk">Disk: 4 GB</span>
  <span class="res-pill res-os">OS: Debian 13</span>
</div>

**Alpine install:**
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/ct/alpine-redis.sh)"
```
<div class="resource-bar">
  <span class="res-pill res-cpu">CPU: 1 cores</span>
  <span class="res-pill res-ram">RAM: 256 MB</span>
  <span class="res-pill res-disk">Disk: 1 GB</span>
  <span class="res-pill res-os">OS: Alpine 3.23</span>
</div>

## Configuration

Config file:
```
Debian /etc/redis/redis.conf | Alpine: /etc/redis.conf
```

## Notes

<div class="info-callout">
  <i class="fas fa-info-circle"></i>
  <div>Redis Configuration: <code>nano /etc/redis/redis.conf</code> or in Alpine: <code>nano /etc/redis.conf</code></div>
</div>

## Links

- [Official Website](https://redis.io/)
- [Documentation](https://redis.io/docs/latest/)

---